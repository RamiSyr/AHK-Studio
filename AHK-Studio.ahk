...
ContextMenu(){
	...
	Loop,%AList%{
		...
		ATitle1:=Trim(SubStr(ATitle,1,InStr(ATitle,"-",0,0,1)-1))
		if(ATitle1=Current(2).file){
			Menu,RCM,Add,Kill Current Program,KillCurrentProgram
			kill:=ID
		}
	}
	Menu,RCM,Show
...
Options(x:=0){
	...
	if(x="startup"){
		...
		v.options[x]:=onoff
		if(x="top_find"){
			this:=MainWin,this.TL:={x:0,y:v.Options.Top_Find?21:0},this.BR:={x:0,y:v.Options.Top_Find?(v.debug?200:0):(v.debug?200:21)},this.Size([1])
