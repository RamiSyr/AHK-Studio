	...
	__New(win:=1){
		...
		*/
		...
		while(ll:=list.item[A_Index-1]),ea:=xml.EA(ll){
			if(ea.type="scintilla"){
				sc:=new s(1,{pos:"x" ea.x " y" ea.y " w" ea.w " h" ea.h}),hwnd:=sc.sc,sc.2277(0) ;,sc.2402(0x08|0x04,40)
				if(ea.file)
		...
		*/
		...
		if(type="scintilla"){
			...
			node:=this.Add(sc.sc,"Scintilla"),Color(sc),sc.2277(0)
			for a,b in space
...
class s{
	...
	__New(window,info){
		...
		this.2663(5),this.2277(0),Color(this)
...
DynaRun(Script,Wait:=true,name:="Untitled"){
	...
	/*
		if(info:=exec.StdERR.ReadAll()){
			m(info,exec.StdERR.ReadAll(),"flan!")
			SetTimer,CheckForError,Off
		}
	*/
	/*
		if(exec.status=0){
			m(exec.StdERR.ReadAll())
			SetTimer,CheckForError,Off
			t()
			return
		}
	...
	*/
	...
	if(!ErrorLevel){
		SetStatus("Script Exited, Code: " exec.status " " (text:=exec.StdERR.ReadAll()),3)
		m(exec.StdOut.ReadAll(),text)
		SetTimer,CheckForError,Off
		return 1
	}else if(!Exec.StdOut.AtEndOfStream){
		/*
...
Gui(){
	...
	if((list:=this.Gui.SN("//win[@win='" win "']/descendant::control")).length){
		...
		return this
	}this.qfhwnd:=this.QuickFind(),sc:=new s(1,{pos:"x0 y0 w100 h100"}),this.Add(sc.sc,"Scintilla"),sc.2277(0),this.test:=sc.sc,this.Pos(),Redraw(),ObjRegisterActive(PluginClass)
