...
FileCheck(file:=""){
	...
	scidate:=20160512235500,XMLFiles:={menus:[20160712045754,"lib/menus.xml","lib\Menus.xml"],commands:[20160508000000,"lib/commands.xml","lib\Commands.xml"]}
...
VarBrowser(){
	...
	if(!WinExist(newwin.id))
		newwin:=new GUIKeep(98),newwin.add("TreeView,w300 h400 gvalue vtreeview AltSubmit hwndtreeview,,wh","Button,gStopDebug,Stop Debugging,y","Button,x+M gRefreshDebugVars,Refresh Variable List,y","Edit,xm w200 vfindvalue,,y","Button,x+M gSearchVar,Search,y"),newwin.show("Variable Browser"),hwnd:=newwin.XML.SSN("//*/@hwnd").text,debug.VarBrowser:=1
	all:=debug.xml.SN("//where|//property|//wait")
	while(aa:=all.item[A_Index-1]),ea:=XML.EA(aa){
		Default("SysTreeView321",98)
		if(ea.where)
			aa.SetAttribute("tv",TV_Add(ea.where))
		else{
			text:=ea.name?ea.name:"Please Wait..."
			if(aa.text)
				text.=" = " debug.Decode(aa.text)
			aa.SetAttribute("tv",TV_Add(text,SSN(aa.ParentNode,"@tv").text))
		}
	}
	return
	searchvar:
	debug.Send("property_get -n " newwin[].findvalue " -i testing")
	return
	98Close:
	98Escape:
	...
	if(A_GuiEvent="+"&&(node:=debug.xml.SSN("//*[@tv='" A_EventInfo "']"))){
		...
		if(SSN(node,"wait"))
			node.SetAttribute("ScanChildren",1),scan:=1
	}
	if(A_GuiEvent="-"&&(node:=debug.xml.SSN("//*[@tv='" A_EventInfo "']")))
		if(SSN(node,"@expanded"))
	...
	if(A_GuiEvent!="Normal"){
		if(scan)
			ScanDebugVars(),scan:=0
		return
	}
	top:=debug.xml.SSN("//*")
	if(node:=SSN(top,"descendant::*[@tv='" A_EventInfo "']")){
		if(SSN(node,"wait"))
			node.SetAttribute("expanded",1),node.SetAttribute("ScanChildren",1),ScanDebugVars()
	}
	return
	StopDebug:
	SetTimer,98Close
...
Display(PopulateVarBrowser:=0){
	...
	while(store:=v.displaymsg.pop()){
		recieve.xml.LoadXML(store),rea:=xml.EA(info:=recieve.SSN("//*"))
		...
		}else if(rea.command="property_get"&&rea.transaction_id){
			if(rea.transaction_id="testing"){
				if(recieve.SN("//property").length=1){
					static ea
					if(node:=recieve.SSN("//*[@type='string']"))
						m(debug.Decode(node.text),"here?")
					else
						SetTimer,CheckProperty,-100
					return
					CheckProperty:
					ea:=xml.EA(recieve.SSN("//property"))
					debug.Send("feature_set -n max_depth -v " ea.children)
					Sleep,40
					debug.Send("feature_set -n max_children -v " ea.numchildren)
					Sleep,40
					debug.Send("property_get -n " ea.fullname " -m " ea.numchildren " -i TestingResult")
					Sleep,% ea.numchildren*40
					debug.Send("feature_set -n max_depth -v 0")
					Sleep,40
					debug.Send("feature_set -n max_children -v 0")
				...
				return
			}else if(rea.transaction_id="TestingResult"){
				recieve.Transform()
				recieve.Transform()
				t(recieve[]),m("Continue"),t()
				return
			}
			/*
				else{
					recieve.Transform()
					recieve.Transform()
					m(rea.transaction_id,"","",recieve[],"here2")
				}
			*/
			xx:=debug.xml,ea:=xml.EA(info),main:=xx.SSN("//*[@where='" ea.transaction_id "']"),all:=SN(info,"//descendant::property")
			while(aa:=all.item[A_Index-1]),ea:=XML.EA(aa){
				if(A_Index=1){
					...
					if(wait:=SSN(master,"descendant::wait")){
						if(tv:=SSN(wait,"@tv").text)
							Default("SysTreeView321",98),TV_Delete(tv)
					...
					if(!next:=SSN(master,"descendant-or-self::property[@fullname='" ea.fullname "']"))
						...
						for a,b in ea
							next.SetAttribute(a,b)
						nea:=xml.EA(next),Default("SysTreeView321",98),TV_Modify(nea.tv,"",nea.name ((text:=debug.Decode(aa.text))?" = " text:""))
			...
			while(nn:=not.item[A_Index-1]),ea:=XML.EA(nn){
				Default("SysTreeView321",98),nn.SetAttribute("tv",TV_Add(ea.name ((text:=debug.Decode(nn.text))?" = " text:"") ,SSN(nn.ParentNode,"@tv").text))
				if(ea.type="object"&&ea.children)
					Default("SysTreeView321",98),xx.Under(nn,"wait",{tv:TV_Add("Please Wait...",SSN(nn,"@tv").text)}),nn.SetAttribute("scope",SSN(nn,"ancestor::*/@where").text)
			}
			ScanDebugVars()
			...
			all:=SN(info,"descendant-or-self::property"),pea:=xml.EA(info),xx:=debug.xml,main:=xx.SSN("//*[@where='" pea.transaction_id "']")
			while(aa:=all.item[A_Index-1]),ea:=XML.EA(aa){
				if(!property:=SSN(main,"descendant::property[@fullname='" ea.fullname "']")){
					property:=main.AppendChild(aa.CloneNode(0))
					if(aa.text)
						property.text:=aa.text
				}else
					for a,b in ea
						property.SetAttribute(a,b)
				if(!tv:=SSN(property,"@tv").text){
					Default("SysTreeView321",98),property.SetAttribute("tv",(parent:=TV_Add(ea.fullname ((text:=debug.Decode(aa.text)?" = " text:"")),SSN(property.ParentNode,"@tv").text,"Vis")))
					...
					}Default("SysTreeView321",98),TV_Modify(prop.tv,"",prop.fullname ((text:=debug.Decode(aa.text))?" = " text:""))
				}if(ea.children&&SSN(property,"@expanded").text)
					property.SetAttribute("ScanChildren",1),property.SetAttribute("id",pea.context)
			}
			ScanDebugVars()
		}else if(rea.command="stack_get"){
			xx:=debug.xml,stack:=recieve.SN("//stack")
			while(ss:=stack.item[A_Index-1]),ea:=XML.EA(ss){
				...
				for a,b in {scan:1,id:ea.level}
					where.SetAttribute(a,b)
				if(!SSN(where,"@tv").text)
					Default("SysTreeView321",98),where.SetAttribute("tv",TV_Add(ea.where))
			}file:=RegExReplace(RegExReplace(URIDecode(stack.filename),"\Qfile:///\E"),"\/","\"),SetPos({file:file,line:stack.lineno-1}),ScanDebugVars()
		}else if(rea.status="stopped"){
			sc.2003(sc.2006,"Execution Complete")
		}else if(rea.status="break"&&rea.reason="ok")
			debug.Send("stack_get")
