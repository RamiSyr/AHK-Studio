...
class debug{
	...
	*/
	...
	Disconnect(){
		...
		v.DebugHighlight:=[],DebugHighlight()
	}
	Accept(){
	...
	while(store:=v.displaymsg.pop()){
		...
		if(info.NodeName="stream"){
			...
			while(ss:=stack.item[A_Index-1]),ea:=XML.EA(ss){
				...
				if(FileExist(filename)&&exist=0){
					...
					/*
						...
						if(v.Options.Select_Current_Debug_Line)
							...
							first:=sc.2152,lines:=sc.2370,half:=Floor(lines/2),NewLine:=(((ea.lineno-1)-half)>0)?(ea.lineno-1)-half:0,sc.2613(NewLine)
					*/
					file:=ea.filename
					/*
				...
				*/
			}DebugHighlight(),debug.Send("context_names -i Context_Names"),sc:=v.debug
...
DebugHighlight(){
	/*
		in tv(){
			if v.DebugHighlight[Current(3).file] (which "should" already have a value...if not use the previous)
				sc.2045(2),sc.2045(3)
		}
		else
			clear out the v.DebugHighlight[] on disconnect and send it here :)
	*/
	sc:=csc(),sc.2045(2),sc.2045(3)
	for a,b in v.DebugHighlight[Current(3).file]{
		...
		if(A_Index=1){
			if(v.Options.Select_Current_Debug_Line)
				sc.2160(sc.2167(b),sc.2136(b))
			else
				first:=sc.2152,lines:=sc.2370,half:=Floor(lines/2),NewLine:=((((b)-half)>0)?(b)-half:0),sc.2613(NewLine)
		}
	}
}