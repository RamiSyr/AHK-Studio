			...
			if(!sc.2102){
				...
				for a,b in new
					if(A_Index>1)
						add.="`n" b
				sc.2207(0xff0000)
				sc.2200(WordStartPos,(main:=start "(" new.1 ")") add)
				;t(start,"flan?")
				;RegExMatch()
				;here now damnit
				;RegExMatch()
				begin:=StrPut(start "(","UTF-8")-1
				end:=StrPut(main,"UTF-8")-2
				pos:=InStr(new.1,",",0,1,comma)
				pos1:=InStr(new.1,",",0,1,comma+1)
				cma:=StrSplit(new.1,",")
				if(pos&&pos1)
...
Backspace(sub:=1){
	...
	Loop,% sc.2570{
		/*
			"" and some other things, also make it so that you can disable it :)
		*/
