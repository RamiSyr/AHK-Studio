...
Debug(text){
	static sc
	if(!sc.sc){
		; mainwin:=new GUIKeep("Debug"),mainwin.Add("Edit,x0 y0 w500 h500,,wh"),mainwin.Show("Debug")
		sc:=new s("Debug",{pos:"x0 y0 w1900 h400"})
		Gui,debug:Show,x0 y0 NA
		; m("here?",sc.sc)
	}text.="`n"
...
DisplayStats(call){
	...
	;sc:=s.ctrl[MainWin.Gui.SSN("//control[@type='Scintilla']/@hwnd").text].2181(0,MainWin.Gui[] "`nControls:`n`n" cl "`n`n" settings[])
	/*
		if(MainWin.gui.SSN("//control[not(@ba) and @type!='Quick Find']")){
			m("OH NO!")
		}
	*/
...
Enter(){
	...
	return sc.2079,sc.2169,MarginWidth(sc),sc.Enable(1)
}
Escape(){
	sc:=csc()
	ControlGetFocus,Focus,% hwnd([1])
...
class GUIKeep{
	...
	__New(win,parent:=""){
		info:=PluginClass.Style(),owner:=WinExist("ahk_id" parent)?parent:x.hwnd(1)
	...
	Show(name,position:=""){
		...
		while,this:=GUIKeep.Showlist.pop(){
			...
			Gui,% this.win ":Show",% position " " pos,% this.name
			this.Size()
			if(this.resize!=1)
...
Notify(csc*){
	...
	if code not in 2001,2006,2008,2010,2014,2022,2016
		return 0
...
class debug{
	...
	ClearXML(){
		rem:=debug.XML.SSN(),rem.ParentNode.RemoveChild(rem)
		return debug.XML.Add("main")
	}
	AddrInfo(){
	...
	Run(filename){
		...
		/*
			do a background kill for the script before trying to run it.
		*/
		new debug()
		;t(big.=command.xml "`n")
		SetTimer,runn,50
	...
	Disconnect(){
		...
		csc().2264(10000000),debug.ClearXML()
		Sleep,200
		DisplayDebugInfo()
	}
	Accept(){
...
Display(){
	static recieve:=new xml("recieve"),total,width,addhotkey,lastid,StoreXML:=[],CV:={1:"Global",0:"Local"},big
	start:=1,store:=""
	sc:=v.debug
	/*
		if(!v.debug.sc)
	...
	*/
	while,v.displaymsg.1{
		...
		if(info:=recieve.SSN("//stream[@type='stderr']")){
			...
			if(in.file&&in.line){
				...
				SetPos({file:in.file,line:in.line}),PluginClass.CallTip(info)
				debug.Disconnect()
			}
			else{
				sc.2003(sc.2006,info "`n"),sc.2025(sc.2006)
		...
		if(init:=recieve.SSN("//init")){
			sc.2004(),sc.2003(0,"Initializing, Please Wait...`n")
			...
			bp:=SN(cexml.Find("//main/@file",Current(2).file),"descendant::info[@type='Breakpoint']")
		...
		if(recieve.SSN("//property")){
			if((property:=recieve.SN("//property")).length){
				if(recieve.SSN("//*/@transaction_id").text="display")
					VarBrowser()
				/*
					if((id:=recieve.SSN("//*/@transaction_id").text)!=lastid)
						TVC.Delete(4),StoreXML:=[],debug.ClearXML()
				*/
				
				; m(recieve.SSN("//*").xml)
				top:=debug.TopXML()
				pea:=recieve.EA("//*")
				tlea:=XML.EA(TopLevel:=SSN(top,"descendant::*[@id='" pea.context "']"))
				node:=TopLevel.AppendChild(recieve.SSN("//*").CloneNode(1))
				if(!tlea.tv)
					TopLevel.SetAttribute("tv",(root:=TVC.Add(4,tlea.name)))
				node.SetAttribute("tv",root)
				; m("Before",node.xml)
				
				object:=SN(node,"descendant::property")
				; m(node.xml,"",object.length)
				...
				list:=[],variablelist:=[],value:=[] ;,object:=recieve.SN("//response/property")
				Gui,97:Default
				
				
				
				
				context:=recieve.SSN("//*/@context").text
				/*
					if(!StoreXML[context]){
						
						t(store)root:=TVC.Add(4,CV[context])
					}
				*/
				/*
					send the receive stuff to the display CloneNode(1)
					have it justify by <response command="context_get" context="1" transaction_id="">
					context is the scope (global:=1 or local:=0)
					split it by treeview parents of Global and Local
					only have the headers if necessary
					Every time it reaches a stop point other than exit, have it get the list of variables
					have a dwell setup so that when you dwell over a word it gets the data from the list above
					have another for builtin vars
					create your own xml to keep track of the treeview items...
					
				*/
				/*
					when it gets to a breakpoint have it do a transaction id of "noshow"
					don't show the var browser when noshow is the id
					but do the rest to get the info!
				*/
				while,oo:=object.item[A_Index-1]{
					ea:=xml.ea(oo),value:=debug.decode(oo.text)
					oo.SetAttribute("tv",TVC.Add(4,(ea.fullname a:=ea.type="object"?"":" = " value),SSN(oo.ParentNode,"@tv").text,(oo.ParentNode.NodeName="response"?"Vis":"")))
					if(ea.type!="object")
				...
				/*
					...
					m(debug.XML[])
				*/
				; ToolTip,% debug.XML[],0,0,3
				if(recieve.SSN("//*/@transaction_id").text!="display"){
					/*
						have it get the list of all the words and go through the doc and highlight (underline) them all
						make it red I think.
						that way the user can see what items have values
					*/
					csc().2264(500)
					DisplayDebugInfo()
				}
			...
			Clipboard:=big.=command.xml "`n"
			ea:=pea:=recieve.EA(command)
			if(pea.command="context_names"){
				GuiControl,97:-Redraw,SysTreeView321
				TVC.Delete(4),top:=debug.ClearXML()
				all:=SN(command,"context"),id:=A_TickCount
				while(a:=all.item[A_Index-1]),ea:=XML.EA(a){
					Sleep,50
					debug.XML.Under(top,"type",{name:ea.name,id:ea.id})
					debug.Send("context_get -c " ea.id " -i " pea.transaction_id)
				}
				if(hotkey:=menus.SSN("//*[@clean='Run_Program']/@hotkey").text)
					sc:=v.debug,sc.2003(sc.2006,"Press " Convert_Hotkey(hotkey) " to continue`n"),sc.2025(sc.2006)
				return
			}
			if(ea.command="stack_get"){
				stack:=recieve.ea(SSN(command,"descendant-or-self::stack"))
				file:=RegExReplace(RegExReplace(URIDecode(stack.filename),"\Qfile:///\E"),"\/","\")
				SetPos({file:file,line:stack.lineno-1})
				debug.Send("context_names")
				stack:=SN(command,"descendant::stack"),CurrentStack:=""
				while(ss:=stack.item[A_Index-1]),ea:=XML.EA(ss){
					CurrentStack.=ea.where "`n" ;Execution Stack
				}
				ToolTip,%CurrentStack%,0,0,4
				/*
					this is either autoexec or sub
					<response command="stack_get" transaction_id="">
						<stack level="0" type="file" filename="file:///D%3A/AHK/Duplicate%20Programs/AHK-Studio/Projects/Scratch%20Pad.ahk" lineno="28" where="flan2 sub"/>
						<stack level="1" type="file" filename="file:///D%3A/AHK/Duplicate%20Programs/AHK-Studio/Projects/Scratch%20Pad.ahk" lineno="24" where="flan sub"/>
						<stack level="2" type="file" filename="file:///D%3A/AHK/Duplicate%20Programs/AHK-Studio/Projects/Scratch%20Pad.ahk" lineno="23" where="Auto-execute thread"/>
					</response>
				*/
			}if(ea.status="stopped"&&ea.command="run"&&ea.reason="ok")
				return sc:=v.debug,debug.Disconnect(),sc.2003(sc.2006,"Execution Complete.")
			disp:="Command: "
			if(ea.status="break")
			...
			if(addhotkey){
				if(hotkey:=menus.SSN("//*[@clean='Run_Program']/@hotkey").text)
					sc.2003(sc.2006,"Press " Convert_Hotkey(hotkey) " to continue`n"),sc.2025(sc.2006)
				else{
	...
	sc:=v.debug
	if(hotkey:=menus.SSN("//*[@clean='Run_Program']/@hotkey").text)
		sc.2004(),sc.2003(sc.2006,"Press " Convert_Hotkey(hotkey) " to continue`n")
	/*
		v.afterbug.Remove(1)
		if(!v.afterbug.1){
...
VarBrowser(){
	...
	; m(newwin.XML[])
	/*
		add a button to change the depth
		also you have the info on how many children a given value has...maybe just get that info on the fly?
		but a button to change the children and
		
		
		max_depth and max_children
		
		or just have a max_depth and max_children edit field?
		I gotta pee... ;#[Debug VarBrowser()]
	*/
	hwnd:=newwin.XML.SSN("//*/@hwnd").text
	TVC.Register(4,hwnd,"value",97)
	return
	value:
	if A_GuiEvent!=Normal
		return
	/*
		Man this needs cleaned up...
	*/
	...
	if value:=v.variablelist[A_EventInfo]{
		ei:=A_EventInfo,newvalue:=InputBox(x.sc().sc,"Current value for " value.variable,"Change value for " value.variable,value.value)
		if ErrorLevel
			return
...
List_Variables(){
	...
	debug.Send("context_names -i Display")
	; debug.Send("context_get") ; -c 1 This will retrieve the current variable scope (local or global)
	/*
...
Debug_Current_Script(){
	...
	*/
	while(bb:=break.item[A_Index-1]),ea:=XML.EA(bb){
		text:=Update({get:ea.filename}),pos:=1
		while(RegExMatch(text,v.Omni_Search.Bookmark,found,pos),pos:=found.Pos(1)+found.len(1)){
			if(!found.len(1))
				Break
			...
			*/
			...
			;nnn:=cexml.Under(node,"info",{type:a,upper:upper(found.1),pos:StrPut(SubStr(text,1,found.Pos(0)),"utf-8"),text:found.1})
			/*
				
			*/
			/*
				if(ea.filename!=lastfile&&(text:=Update({get:ea.filename}))){
					
				}
				lastfile:=ea.filename
			*/
		}
	}
	
	Save(),debug.Run(Current(2).file)
}
Step_Into(){
	if(!debug.socket)
...
Step_Out(){
	...
	if(!debug.socket)
		return m("There is currently no script being debugged")
	debug.Send("stop"),obj:=MainWin.NewCtrlPos:=[],obj.ctrl:=MainWin.Gui.SSN("//*[@type='Debug']/@hwnd").text,obj.win:=hwnd(1),MainWin.Delete()
}
Run_Program(){
	if(!debug.socket)
		return m("Currently no file being debugged"),debug.off()
