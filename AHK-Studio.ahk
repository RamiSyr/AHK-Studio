...
FileCheck(file:=""){
	...
	scidate:=20160512235500,XMLFiles:={menus:[20160715113833,"lib/menus.xml","lib\Menus.xml"],commands:[20160508000000,"lib/commands.xml","lib\Commands.xml"]}
		...
		if(info.NodeName="stream"){
			...
			while(ss:=stack.item[A_Index-1]),ea:=XML.EA(ss){
				...
				if(FileExist(filename)&&exist=0){
					...
					sc.2160(sc.2167(ea.lineno-1),sc.2136(ea.lineno-1)),sc.2043(ea.lineno-1,2)
					file:=ea.filename
					/*
