	...
	while,v.displaymsg.1{
		...
		if(init:=recieve.SSN("//init")){
				...
				if(A_Index=1){
					...
					if(!next:=SSN(master,"descendant-or-self::property[@fullname='" ea.fullname "']")){
						...
						for a,b in ea
							next.SetAttribute(a,b)
						nea:=xml.EA(next)
						Default("SysTreeView321",98),TV_Modify(nea.tv,"",nea.fullname " = " debug.Decode(aa.text))
					}else{
			...
			while(nn:=not.item[A_Index-1]),ea:=XML.EA(nn){
				Default("SysTreeView321",98)
				nn.SetAttribute("tv",TV_Add(ea.fullname " = " debug.Decode(nn.text),SSN(nn.ParentNode,"@tv").text))
			}
			ScanDebugVars()
