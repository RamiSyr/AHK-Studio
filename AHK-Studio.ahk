		...
		if(recieve.SSN("//property")){
			...
			if(pea.command="context_names"){
				...
				while(a:=all.item[A_Index-1]),ea:=XML.EA(a){
					...
					; m(ea.name) ;#[Debug Start Here]
					debug.Send("context_get -c " ea.id " -i " pea.transaction_id)
				}
				return
			}
			if(ea.command="stack_get"){
				stack:=recieve.ea(SSN(command,"descendant-or-self::stack")),file:=RegExReplace(RegExReplace(URIDecode(stack.filename),"\Qfile:///\E"),"\/","\"),SetPos({file:file,line:stack.lineno-1})
				debug.Send("context_names")
				; debug.Send("context_get -c " ea.id) ;#[Debug Get Info but dont show the gui]
