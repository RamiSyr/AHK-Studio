...
v.OmniFind:={Function:"OUm`n)^[\s|}]*((\w|[^\x00-\x7F])+)\((.*)\)(\s*;.*\R){0,}\s*(\{)(\s*;\R){0,}",Class:"Oim`n)^[\s|}]*(class\s+((\w|[^\x00-\x7F])+))(\s*;.*\R){0,}\s*(\{)",Property:"Om`n)^[\s|}]*((\w|[^\x00-\x7F])+)\[(.*)\](\s*;.*\R){0,}\s*(\{)",Label:"UOm`n)^\s*((\w|[^\x00-\x7F])+):[\s|\R][\s+;]?",Hotkey:"OUi`nm)^\s*(((\w|[^\x00-\x7F]|#|!|\^|\+|~|\$|&|<|>|\*)+\s+&\s+)*(\w|[^\x00-\x7F]|#|!|\^|\+|~|\$|&|<|>|\*)+)::",Bookmark:"OU);#\[(.*)\]",Breakpoint:"OU);\*\[(.*)\]",Instance:"OUi)(\w+)\s*:=\s*new\s*(\w+)\("}
;,Variable:"Osm`n)(\w+)\s*:="
v.OmniFindText:={Function:["OUm`n)^[\s|}]*(",")\((.*)\)(\s*;.*\R){0,}\s*(\{)"],Class:["Oim`n)^[\s|}]*(class\s+(","))(\s*;.*\R){0,}\s*(\{)"],Method:["OUm`n)^[\s|}]*(",")\((.*)\)(\s*;.*\R){0,}\s*(\{)"],Property:["Om`n)^[\s|}]*(",")\[(.*)\](\s*;.*\R){0,}\s*(\{)"],Label:["UOm`n)^\s*(","):[\s|\R][\s+;]?"],Bookmark:["OU);#\[(",")\]"],Breakpoint:["OU);\*\[(",")\]"],Hotkey:["OUi`nm)^\s*(\Q","\E)::"],Instance:["OUi).*(",")\s*:=\s*new\s*(\w+)\("]}
v.OmniFindMinimum:={Function:"OUm`n)^[\s|}]*((\w|[^\x00-\x7F])+)\(.*\)",Class:"Oim`n)^[\s|}]*(class\s+((\w|[^\x00-\x7F])+))",Property:"Om`n)^[\s|}]*((\w|[^\x00-\x7F])+)\[(.*)?\]"}
v.OmniFindString:="OUm`n)(?<Function>^[\s|}]*((\w|[^\x00-\x7F])+)\((.*)\)(\s*;.*\R){0,}\s*(\{)(\s*;\R){0,})|(?<Class>^[\s|}]*(class\s+((\w|[^\x00-\x7F])+))(\s*;.*\R){0,}\s*(\{))|(?<Property>^[\s|}]*((\w|[^\x00-\x7F])+)\[(.*)\](\s*;.*\R){0,}\s*(\{))|(?<Label>^\s*((\w|[^\x00-\x7F])+):[\s|\R][\s+;]?)|(?<Hotkey>^\s*(((\w|[^\x00-\x7F]|#|!|\^|\+|~|\$|&|<|>|\*)+\s+&\s+)*(\w|[^\x00-\x7F]|#|!|\^|\+|~|\$|&|<|>|\*)+)::)|(?<Bookmark>;#\[(.*)\])|(?<Breakpoint>;\*\[(.*)\])|(?<Instance>(\w+)\s*:=\s*new\s*(\w+)\()"
/*
	MISC NOT WORKING:
	Undo:
...
*/
file=%1%
ComObjError(0),FileCheck(file),Options("startup"),menus:=new XML("menus","Lib\Menus.xml"),Keywords(),new Omni_Search_Class(),Gui(),DefaultRCM()
	...
	__New(win:=1){
		...
		*/
		...
		for a,b in {border:32,caption:4,menu:15}
			MainWindowClass[a]:=this[a]:=DllCall("GetSystemMetrics",int,b)
		...
		/*
			v.DebugSC:=new s(1,{pos:"w200 h200"})
		*/
		GUI,1:Default
		return this
	}Add(hwnd,type){
		...
		if(xx.SSN("//" top "*[@x=" oea.x+oea.w " and @y=" oea.y "]")&&xx.SSN("//" top "*[@x=" oea.x+oea.w " and @y+@h=" oea.y+oea.h "]")){
			...
			else if(oea.type="Scintilla")
				s.Hidden.push(oea.hwnd):=1,this.SetWinPos(oea.hwnd,0,0,0,0)
			else if(oea.type="Tracked Notes"){
				this.SetWinPos(oea.scintilla,0,0,0,0,ea)
				this.SetWinPos(oea.TreeView,0,0,0,0,ea)
				Redraw()
			}else
		...
		while(ll:=list.item[A_Index-1]),ea:=xml.EA(ll){
			...
			}else if(ea.type="Console"){
				sc:=new s(1,{pos:"x" ea.x " y" ea.y " w" ea.w " h" ea.h}),hwnd:=sc.sc
			}else
		...
		if(ea.type="Tracked Notes"){
			...
			if(ea.vertical){
				DllCall("SetWindowPos",ptr,ea.TreeView,int,0,int,x,int,y,int,w,int,(height:=Round(h*split)),uint,0x0004|0x0010|0x0020)
				DllCall("SetWindowPos",ptr,ea.scintilla,int,0,int,x,int,y+height,int,w,int,h-height,uint,0x0008|0x0004|0x0010|0x0020)
			}else{
				DllCall("SetWindowPos",ptr,ea.TreeView,int,0,int,x,int,y,int,(width:=Round(w*split)),int,h,uint,0x0004|0x0010|0x0020)
				DllCall("SetWindowPos",ptr,ea.Scintilla,int,0,int,x+width,int,y,int,w-width,int,h,uint,0x0008|0x0004|0x0010|0x0020)
			}
			DllCall("RedrawWindow",int,ea.TreeView,int,0,int,0,uint,0x401|0x2)
			DllCall("RedrawWindow",int,ea.Scintilla,int,0,int,0,uint,0x401|0x2)
			return
		}
		...
		this.TopY:=pos.t.y
		/*
			I'm going to have to make the debug window just there at all times.
			have a right click to "close" it but just hide it.
		*/
		if(v.debug)
			this.SetWinPos(this.debug,pos.t.x,(v.Options.top_find?pos.b.y:pos.b.y+21),pos.b.x,200-(v.options.top_find?0:21),"")
		...
		if(type="scintilla"){
			...
			node:=this.Add(sc.sc,"Scintilla"),Color(sc),sc.2277(v.Options.End_Document_At_Last_Line)
			for a,b in space
...
Close(x:=1,all:="",Redraw:=1){
	...
	nodes:=all?files.SN("//main[@file!='Libraries']"):files.SN("//main[@id='" pea.id "']"),Save(3)
	Loop,2
		TVC.Disable(A_Index)
	if(x.length)
		nodes:=x
...
DefaultRCM(){
	...
	for a,b in ["Scintilla","Code Explorer","Project Explorer","Tracked Notes","Toolbar"]{
...
DynaRun(Script,Wait:=true,name:="Untitled"){
	...
	filename:=name,v.debug:=1,MainWin.Size()
	if(!InStr(Script,"m(x*){"))
		Script.="`n" "m(x*){`nfor a,b in x`nlist.=b " cr "`nMsgBox,,AHK Studio,% list`n}"
	if(!InStr(Script,"t(x*){"))
		Script.="`n" "t(x*){`nfor a,b in x`nlist.=b " cr "`nToolTip,% list`n}"
	...
	exec.StdIn.Close(),started:=A_Now
...
ExecScript(){
	...
	v.RunObject[Current(2).file].Terminate()
	SplitPath,file,,dir
	SetWorkingDir,%dir%
	...
	if(v.Exec.Status=0){
		...
		if(InStr(text,"cannot be opened"))
			return m("Do it again but add #Include %A_ScriptDir% to the script",Clipboard:=text)
		exec.Terminate(),sc:=csc()
		info:=StripError(text,"*")
		tv(SSN(files.Find(Current(1),"descendant::file/@file",info.file),"@tv").text),line:=info.line
		Sleep,100
		sc.2160(sc.2128(line),sc.2136(line)),sc.2200(sc.2128(line),text)
	}
	/*
		SetTimer,MonitorExec,100
		time:=A_Now,script:=StrSplit(Current(2).file,"\").pop()
		return
		MonitorExec:
		if(!exec.status){
			SetStatus(script " Running-Time: " A_Now-time,3)
			; t("here?",A_TickCount,exec.status,exec.stdout.ReadAll())
		}else{
			SetStatus(script " Exit Code: " exec.exitcode,3)
			m(exec.StdOut.ReadAll(),exec.StdOut.ReadAll())
			SetTimer,MonitorExec,Off
		}
	*/
	return
}
...
Extract(mainfile,TrackNew:=""){
	...
	*/
	...
	ExtractNext:
	id:=GetID(),TrackNew.Under(TrackNew.SSN("//*"),"new",,file),fff:=FileOpen(file,"R"),encoding:=fff.encoding,text:=fff.read(fff.length),fff.Close(),dir:=Trim(dir,"\")
...
FileCheck(file:=""){
	...
	scidate:=20160512235500,XMLFiles:={menus:[20160704112650,"lib/menus.xml","lib\Menus.xml"],commands:[20160508000000,"lib/commands.xml","lib\Commands.xml"]}
...
Hotkeys(win:=1,keys:=""){
	...
	if(IsFunc(clean)||IsLabel(clean))
		SetTimer,%clean%,-1
	/*
		else if(clean="Command_Help"){
			this:=MainWin
			v.Options.Top_Find:=!v.Options.Top_Find
			settings.Add("options").SetAttribute("Top_Find",v.Options.Top_Find)
			this.TL:={x:0,y:v.Options.Top_Find?21:0}
			this.BR:={x:0,y:v.Options.Top_Find?(v.debug?200:0):(v.debug?200:21)}
			this.Size([1])
			Redraw()
		}
		else if(clean="duplicate_line"){
			global debug
			this:=MainWin
			v.debug:=v.debug?0:1
			this.BR:={x:0,y:v.Options.Top_Find?(v.debug?200:0):(v.debug?200:21)}
			if(v.debug){
				sc:=new s(1,{pos:"x0 y0 w0 h0"})
				this.debug:=sc.sc,sc.2181(0,"I R A Debugger Window`n`nGui,Color,0xff00ff,0xff00ff`n`nlol"),Color(sc)
			}else
				if(this.debug)
					DllCall("DestroyWindow",ptr,this.debug)
			this.Size(["Resize"]),this.Fix(),Redraw()
		}
	*/
	/*
		else if(clean="Next_Found"){
			m(MainWin.Gui[])
			MainWin.Delete()
		}
	*/
...
Notify(csc*){
	...
	static codeget:={2001:{ch:4},2005:{ch:4,mod:5},2006:{position:3,mod:5},2007:{updated:22},2008:{code:2,position:3,modType:6,text:7,length:8,linesadded:9,line:13,fold:14,prevfold:15},2010:{position:3},2011:{position:3},2014:{position:3,ch:4,text:7,listtype:17,Method:23},2019:{position:3,mod:5},2021:{position:3},2022:{position:3,ch:4,text:7,method:23},2027:{position:3,mod:5}},PreType:=[]
	...
	if code not in 2001,2006,2008,2010,2014,2022
		return 0
...
Options(x:=0){
	...
	if(x="startup"){
		...
		if(x="top_find"){
			...
			this.BR:={x:0,y:v.Options.Top_Find?(v.debug?200:0):(v.debug?200:21)}
			this.Size([1])
			Redraw() ;,RefreshThemes(),DllCall("RedrawWindow",int,hwnd(1),int,0,int,0,uint,0x1)
		}
...
Save_Untitled(node,ask:=1){
	...
	if(text!=template){
		...
		*/
		...
		m("just remove this un-changed untitled file")
	}
}
Save(option=""){
	sc:=csc()
