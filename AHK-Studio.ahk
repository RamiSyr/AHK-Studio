...
Testing(){
	;return Display_Hotkeys()
	v.debug.Enable(1)
	; m(WinExist("ahk_id" v.debug.sc))
	/*
	...
	*/
	while,v.displaymsg.1{
		...
		if(info:=recieve.SSN("//stream[@type='stderr']")){
			; m(recieve[])
			info:=debug.decode(info.text),total.=info "`n",sc.2003(sc.2006,info "`n"),sc.2025(sc.2006),in:=striperror(info,v.debugfilename)
			if(in.file&&in.line){
		...
		if(recieve.SSN("//property")){
			if((property:=recieve.SN("//property")).length){
				...
				*/
				while,oo:=object.item[A_Index-1]{
					...
					; list[ea.fullname]:=TVC.Add(4,ea.fullname a:=ea.type="object"?"":" = " value,root)
					oo.SetAttribute("tv",TVC.Add(4,ea.fullname a:=ea.type="object"?"":" = " value,SSN(oo.ParentNode,"@tv").text))
					if(ea.type!="object")
					...
					/*
						...
						while,des:=descendant.item[A_Index-1]{
							...
							; list[ea.fullname]:=TVC.Add(4,ea.fullname a:=ea.type="object"?"":" = " value,list[prev:=SubStr(ea.fullname,1,InStr(ea.fullname,".",0,0)-1)],"Sort")
							; tv1:=SSN(oo.ParentNode,"@tv").text?SSN(oo.ParentNode,"@tv").text:root
							des.SetAttribute("tv",TVC.Add(4,ea.fullname a:=ea.type="object"?"":" = " value,SSN(des.ParentNode,"@tv").text,"Sort"))
							if(ea.type!="object")
								variablelist[list[ea.fullname]]:={value:value,variable:ea.fullname}
						}
					*/
				...
				*/
				...
				/*
					...
					debug.XML.Transform()
					debug.XML.Transform()
					m("This is what I got:",debug.XML[])
					return
				*/
			}
			...
			if(ea.command="stack_get")
				stack:=recieve.ea(SSN(command,"descendant-or-self::stack")),file:=RegExReplace(RegExReplace(URIDecode(stack.filename),"\Qfile:///\E"),"\/","\"),SetPos({file:file,line:stack.lineno-1}),debug.Send("context_names")
			...
			if(addhotkey){
				if(hotkey:=menus.SSN("//*[@clean='Run_Program']/@hotkey").text)
					disp.="`nPress " Convert_Hotkey(hotkey) " to continue"
				else{
	...
	v.debug.2003(v.debug.2006,"`nPress " Convert_Hotkey(menus.SSN("//*[@clean='Run_Program']/@hotkey").text) " to continue")
	/*
		v.afterbug.Remove(1)
		if(!v.afterbug.1){
