...
FileCheck(file:=""){
	...
	scidate:=20160512235500,XMLFiles:={menus:[20160715200225,"lib/menus.xml","lib\Menus.xml"],commands:[20160508000000,"lib/commands.xml","lib\Commands.xml"]}
...
class debug{
	...
	__New(){
		...
		debug.filename:=Current(2).file,debug.id:=Current(2).id,debug.Hotkeys(1)
	}Hotkeys(State){
		state:=state?"On":"Off"
	...
	*/
	...
	Disconnect(){
		...
		v.DebugHighlight:=[],DebugHighlight(),debug.Hotkeys(0)
	}
	Accept(){
...
VarBrowser(){
	...
	if(!WinExist(newwin.id))
		debugwin:=newwin:=new GUIKeep(98),newwin.add("TreeView,w300 h400 gvalue vtreeview AltSubmit hwndtreeview,,wh","ListView,w300 r4 AltSubmit gVBGoto,Stack|File|Line,wy","Button,gStopDebug,Stop Debugging,y"),newwin.show("Variable Browser"),hwnd:=newwin.XML.SSN("//*/@hwnd").text,debug.VarBrowser:=1
	...
	if(A_GuiEvent~="Normal|I"){
		...
		if(tv:=SSN(files.Find("//file/@file",file),"@tv").text){
			tv(tv),sc:=csc()
			Sleep,40
			sc.2160(sc.2128(line-1),sc.2136(line-1))
		}
	}
	return
	value:
...
Display(PopulateVarBrowser:=0){
		...
		if(info.NodeName="stream"){
			...
			while(ss:=stack.item[A_Index-1]),ea:=XML.EA(ss){
				...
				if(FileExist(filename)&&exist=0){
					...
					file:=ea.filename,scope:=ea.where="Auto-execute thread"?"Global":RegExReplace(ea.where,"(\..*)"),xx.Add("master",{scope:scope}),exist:=1
	...
	while(info:=v.afterbug.pop())
		debug.Send(info)
	InsertDebugMessage(),v.ready:=1,debug.Focus()
	return
	GetContextInfo:
	debug.Send("stack_get")
	return
