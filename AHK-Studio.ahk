...
class debug{
	...
	__New(){
		...
		v.ddd:=this,debug.id:=Current(3).id
	}
	TopXML(){
...
Display(){
	static recieve:=new xml("recieve"),total,width,addhotkey,lastid,StoreXML:=[],CV:={1:"Global",0:"Local"}
		...
		*/
		...
		if(recieve.SSN("//property")){
			...
			if(ea.command="stack_get"){
				stack:=recieve.ea(SSN(command,"descendant-or-self::stack")),file:=RegExReplace(RegExReplace(URIDecode(stack.filename),"\Qfile:///\E"),"\/","\"),SetPos({file:file,line:stack.lineno}),debug.Send("context_names"),stack:=SN(command,"descendant::stack"),CurrentStack:=""
				while(ss:=stack.item[A_Index-1]),ea:=XML.EA(ss)
					CurrentStack.=ea.where "`n" ;Execution Stack
				ToolTip,%CurrentStack%,0,0,4 ;#[This needs to go into a GUI to show the stack (maybe even with the variables) maybe a separate TreeView]
...
InsertDebugMessage(){
	sc:=v.debug
	if(hotkey:=menus.SSN("//*[@clean='Run_Program']/@hotkey").text){
		sc.2003(sc.2006,"Press " Convert_Hotkey(hotkey) " to continue")
		Sleep,10
	}
	else{
		msg:="Click Here to Continue",end:=sc.2006(),sc.2003(end,msg)
		...
		msg:=" : Click Here to Show the Variable List`n",end:=sc.2006(),sc.2003(end,msg)
		sc.2051(151,0xff0000),sc.2032(end+3,151),sc.2033(StrPut(msg,"UTF-8")-1,151),sc.2409(151,1)
	}
	
	/*
		sc.2686(end,sc.2006),pos:=sc.2197(36,"Click here to open the Variable List")
		sc.2160(pos,pos+36)
	*/
	sc.2025(sc.2006)
}
VarBrowser(){
	...
	if(!WinExist(newwin.id)){
		/*
			add in the TVC for this as well to make it easy to populate it
		*/
	...
	*/
	...
	if value:=v.variablelist[A_EventInfo]{
		ei:=A_EventInfo,newvalue:=InputBox(csc().sc,"Current value for " value.variable,"Change value for " value.variable,value.value)
		if ErrorLevel
			return
