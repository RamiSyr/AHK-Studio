...
Toolbar_Editor(control){
	...
	nw.Add("ComboBox,gtesearch w460 vedit gTEFindTV,,w"
		 ,"ListView,xm w260 h200 gTESelect AltSubmit,Toolbars,h"
		 ,"TreeView,x+0 w200 h200 Checked AltSubmit,,wh"
		 ,"ListView,x+M200 yM w" width " h180 icon Section gSelectIcon AltSubmit,Icon,xh")
	ControlGet,hwnd,hwnd,,SysListView322,% hwnd(["Toolbar_Editor"])
	new Icon_Browser(nw,hwnd,"Toolbar_Editor","xy",300,"Toolbar_Editor","Toolbar_Editor"),nw.add("Button,xm gTEHighlight,Highlight Color,y","Button,x+M gNextChecked,&Next Button,y"),nw.show("Toolbar Editor"),total:="",cross:=[],Default("SysTreeView321","Toolbar_Editor"),all:=menus.SN("//main/descendant::*")
	while,aa:=all.item[A_Index-1],ea:=xml.ea(aa){
		if(aa.HasChildNodes())
	...
	TESelect:
	/*
		update the checked items here
	*/
	Default("SysListView321","Toolbar_Editor"),LV_GetText(id,LV_GetNext())
	if(id!=LastID){
		Gosub,ResetToolbar
		DeSelect:=settings.SN("//toolbar/bar[@id='" LastID "']/descendant::button")
		...
		LastID:=id,oea:=xml.EA(OControl:=MainWin.GUI.SSN("//*[@id='" id "']")),color:=settings.Get("//Toolbar_Editor/@highlight",0xEE00AA),LastID:=id
