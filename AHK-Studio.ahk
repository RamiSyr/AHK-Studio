...
Display(PopulateVarBrowser:=0){
	...
	while(store:=v.displaymsg.pop()){
		...
		if(info.NodeName="stream"){
			...
			sc:=csc(),stack:=receive.SN("//stack"),sc.2045(2),sc.2045(3)
			while(ss:=stack.item[A_Index-1]),ea:=XML.EA(ss){
				if(A_Index=1){
					...
					*/
					scope:=ea.where="Auto-execute thread"?"Global":RegExReplace(ea.where,"(\..*)"),xx.Add("master",{scope:scope})
					if(WinExist(debugwin.id)){
						...
						while(ss:=scope.item[A_Index-1]),ea:=XML.EA(ss){
							LV_Add("",ea.where,RegExReplace(RegExReplace(URIDecode(ea.filename),"file:\/\/\/"),"\/","\"),ea.lineno)
						}Loop,3
							LV_ModifyCol(A_Index,"AutoHDR")
