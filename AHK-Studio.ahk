...
FileCheck(file:=""){
	...
	scidate:=20160512235500,XMLFiles:={menus:[20160715190929,"lib/menus.xml","lib\Menus.xml"],commands:[20160508000000,"lib/commands.xml","lib\Commands.xml"]}
...
class debug{
	...
	__New(){
		...
		debug.filename:=Current(2).file
		if(!debug.id:=Current(2).id){
			debug.filename:="D:\AHK\Fucks Shit Up.ahk" ;:="D:\AHK\Duplicate Programs\AHK-Studio\Projects\Scratch Pad.ahk"
			ea:=xml.EA(cexml.Find("//file/@file",(debug.filename)))
			debug.id:=ea.id
		}
	}
	TopXML(){
	...
	Send(message){
		...
		if(sent&&mm!="stop"){
			...
			While(debug.wait){
				...
				if(debug.socket<1)
					return
				if(A_Index>20){
					t("Receive buffer jammed. Attempting to clear it"),debug.wait:=0,v.ready:=1,debug.Receive()
					Break
		}}}
		return
	}Receive(){
		...
		While(r<length){
			...
			if(r<1){
				...
				t(r,socket,length,received,"An error occured",error,"Possible reasons for the error:","1.  Sending OutputDebug faster than 1ms per message","2.  Max_Depth or Max_Children value too large","time:2")
			}
			if(r<length)
				Sleep,5
			if(A_Index>10){
...
VarBrowser(){
	...
	if(!WinExist(newwin.id))
		debugwin:=newwin:=new GUIKeep(98),newwin.add("TreeView,w300 h400 gvalue vtreeview AltSubmit hwndtreeview,,wh","ListView,w300 r4 AltSubmit,Stack|File|Line,wy","Button,gStopDebug,Stop Debugging,y"),newwin.show("Variable Browser"),hwnd:=newwin.XML.SSN("//*/@hwnd").text,debug.VarBrowser:=1
...
Display(PopulateVarBrowser:=0){
	...
	while(store:=v.displaymsg.pop()){
		...
		if(info.NodeName="stream"){
			...
			sc:=csc(),sc.2045(2),sc.2045(3),sc:=v.debug,sc.2003(sc.2006,"Execution Complete"),sc.2025(sc.2006)
			...
			sc:=csc(),stack:=receive.SN("//stack"),sc.2045(2),sc.2045(3),exist:=0
			while(ss:=stack.item[A_Index-1]),ea:=XML.EA(ss){
				...
				if(FileExist(filename)&&exist=0){
					...
					sc.2043(ea.lineno-1,2)
					if(v.Options.Select_Current_Debug_Line)
						sc.2160(sc.2167(ea.lineno-1),sc.2136(ea.lineno-1))
					else
						first:=sc.2152,lines:=sc.2370,half:=Floor(lines/2),NewLine:=(((ea.lineno-1)-half)>0)?(ea.lineno-1)-half:0
					sc.2613(NewLine)
					file:=ea.filename
					/*
						have this do the FileExist(file)
						cause h adds in virtual shit...
						if properties start getting put into the wrong scopes{
							add the current time to the transaction_id so that it is the same throughout and associate the time
							with the master scope and if there is more than 1 scope add it.
						}
					*/
					scope:=ea.where="Auto-execute thread"?"Global":RegExReplace(ea.where,"(\..*)"),xx.Add("master",{scope:scope}),exist:=1
					if(WinExist(debugwin.id)){
						...
						while(ss:=scope.item[A_Index-1]),ea:=XML.EA(ss){
							LV_Add("",ea.where,filename,ea.lineno-1)
						}Loop,3
							LV_ModifyCol(A_Index,"AutoHDR")
				}}else if(ea.filename=file)
					sc.2043(ea.lineno-1,3)
			}debug.Send("context_names -i Context_Names"),sc:=v.debug
			if(!v.Options.Disable_Focus_Studio_On_Debug_Breakpoint)
				WinActivate,% hwnd([1])
...
ScanChildren(){
	...
	exp:=xx.SN("//descendant::*[@expanded=1]")
