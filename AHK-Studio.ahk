...
global v:=[],MainWin,settings:=new XML("settings","lib\Settings.xml"),files:=new XML("files"),Positions:=new XML("positions","lib\Positions.xml"),cexml:=new XML("cexml"),History:=new XML("HistoryXML"),vversion:=new XML("versions","lib\Versions.xml"),commands,menus,scintilla,TVC:=new EasyView(),RCMXML:=new XML("RCM","lib\RCM.xml"),TNotes
if(!settings[]){
	Run,lib\settings.xml
	m("Oh boy...check the settings file to see what's up.")
}
v.LineEdited:=[],v.LinesEdited:=[],v.RunObject,v.OmniFind:={Function:"OUm`n)^[\s|}]*((\w|[^\x00-\x7F])+)\((.*)\)(\s*;.*\R){0,}\s*(\{)(\s*;\R){0,}",Class:"Oim`n)^[\s|}]*(class\s+((\w|[^\x00-\x7F])+))(\s*;.*\R){0,}\s*(\{)",Property:"Om`n)^[\s|}]*((\w|[^\x00-\x7F])+)\[(.*)\](\s*;.*\R){0,}\s*(\{)",Label:"UOm`n)^\s*((\w|[^\x00-\x7F])+):[\s|\R][\s+;]?",Hotkey:"OUi`nm)^\s*(((\w|[^\x00-\x7F]|#|!|\^|\+|~|\$|&|<|>|\*)+\s+&\s+)*(\w|[^\x00-\x7F]|#|!|\^|\+|~|\$|&|<|>|\*)+)::",Bookmark:"OU);#\[(.*)\]",Breakpoint:"OU);\*\[(.*)\]",Instance:"OUi)(\w+)\s*:=\s*new\s*(\w+)\("},v.OmniFindText:={Function:["OUm`n)^[\s|}]*(",")\((.*)\)(\s*;.*\R){0,}\s*(\{)"],Class:["Oim`n)^[\s|}]*(class\s+(","))(\s*;.*\R){0,}\s*(\{)"],Method:["OUm`n)^[\s|}]*(",")\((.*)\)(\s*;.*\R){0,}\s*(\{)"],Property:["Om`n)^[\s|}]*(",")\[(.*)\](\s*;.*\R){0,}\s*(\{)"],Label:["UOm`n)^\s*(","):[\s|\R][\s+;]?"],Bookmark:["OU);#\[(",")\]"],Breakpoint:["OU);\*\[(",")\]"],Hotkey:["OUi`nm)^\s*(\Q","\E)::"],Instance:["OUi).*(",")\s*:=\s*new\s*(\w+)\("]},v.OmniFindMinimum:={Function:"OUm`n)^[\s|}]*((\w|[^\x00-\x7F])+)\(.*\)",Class:"Oim`n)^[\s|}]*(class\s+((\w|[^\x00-\x7F])+))",Property:"Om`n)^[\s|}]*((\w|[^\x00-\x7F])+)\[(.*)?\]"},v.OmniFindString:="OUm`n)(?<Function>^[\s|}]*((\w|[^\x00-\x7F])+)\((.*)\)(\s*;.*\R){0,}\s*(\{)(\s*;\R){0,})|(?<Class>^[\s|}]*(class\s+((\w|[^\x00-\x7F])+))(\s*;.*\R){0,}\s*(\{))|(?<Property>^[\s|}]*((\w|[^\x00-\x7F])+)\[(.*)\](\s*;.*\R){0,}\s*(\{))|(?<Label>^\s*((\w|[^\x00-\x7F])+):[\s|\R][\s+;]?)|(?<Hotkey>^\s*(((\w|[^\x00-\x7F]|#|!|\^|\+|~|\$|&|<|>|\*)+\s+&\s+)*(\w|[^\x00-\x7F]|#|!|\^|\+|~|\$|&|<|>|\*)+)::)|(?<Bookmark>;#\[(.*)\])|(?<Breakpoint>;\*\[(.*)\])|(?<Instance>(\w+)\s*:=\s*new\s*(\w+)\()"
;,Variable:"Osm`n)(\w+)\s*:="
/*
	MISC NOT WORKING:
	Undo:
...
OR PERFORMANCE OF THIS SOFTWARE. 
	...
	Setup(11),Hotkeys(11,{"Esc":"11Close"}), Version:="1.002.38"
	Gui,Margin,0,0
...
Testing(){
	...
	*/
	/*
		...
		save the last import directory by project
	*/
...
Activate(a,b,c,d){
	if(a&&v.options.Check_For_Edited_Files_On_Focus=1)
		Check_For_Edited()
	t(a,v.options.Check_For_Edited_Files_On_Focus)
	csc().2400
	return 0
}
			...
			for a,b in {Function:v.OmniFind.Function,Property:v.OmniFind.Property}{
				...
				while(RegExMatch(FileText,b,found,pos)),pos:=found.pos(1)+found.len(1){
					...
					if(this.InComment(found)){
						/*
							if(found.1="Delete"){
								v.CommentArea.Transform()
								v.CommentArea.Transform()
								t(found.0,v.CommentArea[])
							}
						*/
						Continue
					}
...
Class MainWindowClass{
	static keep:=[]
	__New(win:=1){
		;obj:=new MainWindowClass() this is to allow studio to see obj as an object for MainWindowClass
		...
		*/
		...
		;Gui,98:Show,% "x" (x+w-v.Border)-(300) " y" y+v.caption+v.menu+v.Border+(v.options.top_find?v.qfh
		...
		if((this=18||a="Update")&&!obj.mousedown){
			...
			if(obj.ResizeType=1){
				if((list:=obj.GUI.SN("//*[@win=1]descendant::control[(@x+-4<='" x "' and @x+@w+4>='" x "')and(@y+-4<='" y "' and @y+@h+4>='" y "')]")).length>1)
					left:=obj.GUI.SSN("//win[@win='1']/descendant::control[@x+-4<='" x "' and @x+4>='" x "']").xml,top:=obj.GUI.SSN("//win[@win='" A_Gui "']/descendant::control[@y+-4<='" y "' and @y+4>='" y "']").xml
				DllCall("SetCursor","UInt",list.length>2?obj.cursall:top?obj.cursns:obj.cursew)
			}else if(obj.ResizeType=2)
				DllCall("SetCursor","UInt",(obj.GUI.SSN("//*[@vertical]")?obj.cursns:obj.cursew))
		}else
		...
		this.NewCtrlPos:=[]
		this.NewCtrlPos.y:=Round((y+h)*.75)
		this.NewCtrlPos.ctrl:=sc.sc
		this.Split("Below","Debug")
		...
		GUI,+parent1 -Caption -Border hwndhwnd
		GUI,Margin,0,0
		...
		for a,b in ["Regex","Case Sensitive","Greed","Multi-Line"]{
			GUI,Add,Checkbox,% "x+M hwndregex gRegexSettings hwndcontrol " (A_Index=1?"y4 x+5":"") (v.Options[clean(b)]?" Checked":""),%b%
		...
		while(ll:=list.item[A_Index-1]),ea:=xml.EA(ll){
			if(ea.type="scintilla"){
				...
				if(ea.file)
					tv(SSN(files.find("//file/@file",ea.file),"@tv").text,{sc:sc.sc})
		...
		if(obj.ResizeType=1){
			...
			}obj.Fix(),obj.ChangePointer("Update")
		}else if(obj.ResizeType=2){
		...
		if(ea.type="Scintilla"){
			...
			this.Hidden.push(ea.hwnd),this.SetWinPos(ea.hwnd,0,0,0,0,ea)
		}
		if(ea.type="Toolbar")
		...
		if(new="scintilla"){
			...
			tb:=new ToolBar(1,"x" ea.x " y" ea.y " w" ea.w " h" ea.h,id,node),NewHwnd:=tb.hwnd,node.SetAttribute("toolbar",tb.tb)
...
class s{
	...
	__New(window,info){
		...
		this.2663(5),this.2277(v.Options.End_Document_At_Last_Line),Color(this)
		this.2402(0x04|0x01,140)
		this.2403(0x04|0x01,40)
		this.2359(0x1|0x2|0x800|0x400) ;|0x100000|0x800)
		Color(this)
		return this
	}__Get(x*){
...
Class Toolbar{
	...
	__New(win,pos,id:="",node:=""){
		...
		Gui,%b%:+parent1 -Caption -Border +0x400000 hwndhwnd
		...
		settings.SSN("//toolbar/bar[@id='" this.id "']/descendant::button[@id='" id "']").SetAttribute("icon",icon)
	}IL(icon="",file=""){
...
Close(x:=1,all:="",Redraw:=1){
	parent:=Current(1),pea:=xml.EA(parent)
	nodes:=all?files.SN("//main[@file!='Libraries']"):files.SN("//main[@id='" pea.id "']")
	...
	Loop,2
		TVC.Enable(A_Index)
	if(tv:=files.SSN("//main/file/@tv").text)
		tv(tv)
	else
		New()
}
...
Color(con:=""){
	...
	con.2080(2,8),con.2082(2,0xff00ff),con.2636(1),con.2516(1),con.2458(2),con.2051(35,0xff00ff),con.2373(settings.Get("//gui/@zoom",0))
	for a,b in options
		if(v.options[a])
			con[b](b)
	...
	}con.4004("fold",[1]),con.2680(3,8),con.2080(8,1),con.2082(8,0xff00ff),con.2242(4,1),con.2240(4,5),con.2110(1)
	con.2080(6,14)
	/*
		split out the indicators and have their own area in here
	*/
	con.2082(6,0xC08080),con.2198(v.Options.Match_Any_Word?0:0x2)
	con.2080(3,14)
	con.2680(3,6)
...
ContextMenu(){
	...
	if(SSN(node,"@name").text="Project Explorer"){
	...
	if(IsLabel(clean)||IsFunc(clean))
		...
		;m(settings.SSN("//toolbar/bar[@id='" MainWin.Gui.SSN("//*[@hwnd='" MainWin.NewCtrlPos.ctrl "']/@id").text "']").xml)
		;gui.SSN("//*[@id='" bar "' and @type='Toolbar']")
...
DefaultRCM(){
	static all:={Scintilla:"Undo,Redo,Copy,Cut,Paste,Select All,Close,Delete,Open,Open Folder,Omni Search"
		    ...
		    ,Toolbar:"Edit Toolbar,Small Icons"
...
Duplicates(){
	...
	if(dup.MaxIndex()>1){
		for a,b in dup
			sc.2686(b,sc.2006),sc.2504(b,StrLen(search)),found++
...
/*
	DynaRun(Script,debug=0){
		...
		Call:=Chr(34) A_AhkPath Chr(34) " /ErrorStdOut /CP65001 " Chr(34) "\\.\pipe\" Name Chr(34),Shell:=ComObjCreate("WScript.Shell")
...
DynaRun(Script,Wait:=true,name:="Untitled"){
	...
	/*
		if(!exec.status){
			while((text:=Exec.StdOut.Read(1))!="")
				total.=text
			t(total,exec.stdout.length())
		}
	*/
	/*
		if there is anything in StdOut
			have it open up the debug window and display it
		
	*/
	/*
		if(text:=exec.StdOut.Read(1)){
			while(text1:=exec.StdOut.Read(1)){
				t(A_TickCount)
				text.=text1
			}
			m(text)
	...
	*/
	...
	if(!ErrorLevel){
		SetStatus("Script Exited, ExitCode: " exec.ExitCode " " (text:=exec.StdERR.ReadAll()),3)
		SetTimer,CheckForError,Off
		return 1
	}else{
		t(exec.stdout.Readline())
		/*
			t(all.=exec.StdOut.ReadAll()) <---freaks out when still running
		*/
		return 1
	}
...
Enter(){
	...
	if(InStr(focus,"scintilla")){
		...
		order:=[],sc.2078,group:=0,sc.Enable()
		Loop,% sc.2570{
	...
	while(aa:=all.item[A_Index-1]),ea:=xml.ea(aa){
		if(aa.NodeName="folder"){
			...
			if(SSN(aa.ParentNode,"@file").text="Libraries"){
				if(!cexml.SSN("//main[@file='Libraries']"))
					Libraries:=TVC.Add(1,"Libraries",0)
				aa.SetAttribute("tv",TVC.Add(2,v.options.Hide_File_Extensions?ea.nne:ea.filename,Libraries))
...
FileCheck(file:=""){
	base:="https://raw.githubusercontent.com/maestrith/AHK-Studio/master/"
	scidate:=20160512235500,XMLFiles:={menus:[20160711163507,"lib/menus.xml","lib\Menus.xml"],commands:[20160508000000,"lib/commands.xml","lib\Commands.xml"]}
...
Gui(){
	...
	if(!settings.SSN("//autoadd")){
		...
		settings.Add("options",{Full_Auto:1}),Options("Startup")
...
class GUIKeep{
	...
	Show(name,position:="",NA:=0){
		static defpos,pos
		defpos:=position,this.GetPos(),pos:=this.resize=1?"":"AutoSize",this.name:=name
		if(this.resize=1)
			Gui,% this.win ":+Resize"
...
Index_Lib_Files(){
	...
	rem:=cexml.SSN("//lib")
	rem.ParentNode.RemoveChild(rem)
	main:=files.Add("main",{file:"Libraries",id:GetID()},,1)
...
Jump_To(type){
	...
	if(node:=cexml.SSN(cexml.Find("//main[@file",current(2).file),"descendant::*[@type='" type "' and @upper='" word "']"))
		cexmlsel(node)
}Jump_To_Function(){
	Jump_To("Function")
}Jump_To_Include(){
...
NewIndent(indentwidth:=""){
	...
	*/
	...
	selpos:=posinfo()
	lock:=[],block:=[],aa:=ab:=braces:=0,code:=StrSplit(codetext,"`n"),aaobj:=[],specialbrace:=0
	totalcount:=0
	for a,text in code{
		totalcount++
		text:=Trim(text,"`t ")
...
Notify(csc*){
	...
	if(code=2016){
		...
		while(ll:=list.item[A_Index-1]),ea:=XML.EA(ll){
			info:=ea.type="object"?"Object: Use List Variables (Alt+M LV) to see more info":info.=SSN(ll,"ancestor::*/@name").text " = " debug.decode(ll.text) "`n"
		}if(info){
			line:=sc.2166(pos)
			end:=sc.2136(line)
...
Omni_Search(start=""){
	...
	newwin:=new GUIKeep(20),newwin.Add("Edit,goss w200 vsearch ReadOnly,,w","ListView,w200 h100 -hdr -Multi gosgo,Menu C|A|1|2|R|index,wh")
	Gui,20:-Caption
	Hotkeys(20,{up:"omnikey",down:"omnikey",PgUp:"omnikey",PgDn:"omnikey","^Backspace":"deleteback",Enter:"osgo"})
	Gui,1:-Disabled
	newwin.Show("Omni-Search") ;,"x" x " y" (Round(y+(h*.75))) " w" w " h" Round(h*.25))
	WinWait,% hwnd([20])
	GuiControl,20:,Edit1,%start%
	SendMessage,0xB1,% StrLen(start),% StrLen(start),Edit1,% hwnd([20])
	GuiControl,20:-ReadOnly,Edit1
	oss:
	break:=1
...
Options(x:=0){
	static list:={Virtual_Space:[2596,3],End_Document_At_Last_Line:2277,Show_EOL:2356,Show_Caret_Line:2096,Show_Whitespace:2021,Word_Wrap:2268,Hide_Indentation_Guides:2132,Center_Caret:[2403,0x04|0x08],Disable_Word_Wrap_Indicators:2460,Hide_Horizontal_Scrollbars:2130,Hide_Vertical_Scrollbars:2280},Disable,options,other
	if(x="startup"){
		v.options:=[]
...
QF(x:=0){
	...
	GuiControl,,% v.qfhwnd[A_ThisLabel],%onoff%
...
RemoveContent(text,position:=""){
	/*
		t("this needs adjusted","time:2")
		functions for methods, and such
	*/
	return m("Removed:",text,"Deal with it.")
	t("NEEDS FIXED! Functions make sure they are not in a class use the while(line:=sc.2225(line)) method" text,"time:2")
	sc:=csc()
	/*
		parent:=cexml.Find("//main/@file",Current(2).file)
		current:=cexml.Find(parent,"descendant::file/@file",Current(3).file)
	*/
	parent:=Current(7),current:=Current(5)
	all:=sc.GetUNI()
	tick:=A_TickCount
	Enable("SysTreeView322")
	for a,b in v.OmniFind{
		pos:=1
		while(RegExMatch(text,b,found,pos)),pos:=found.pos(1)+found.len(1){
			if(!found.len(1))
				Break
			/*
				t(A_ThisFunc,A_TickCount,"time:1",found.1,found.len(1))
			*/
			if(a~="Bookmark|Breakpoint|Instance"){
				if(node:=SSN(current,"*[@type='" a "' and @text='" found.1 "']")){
					ea:=xml.EA(node)
					if(ea.cetv)
						TVC.Delete(2,ea.cetv)
					node.ParentNode.RemoveChild(node)
				...
				if(a="Class"){
					class:=SSN(current,"//*[@upper='" Upper(found.2) "']"),all:=SN(class,"descendant::*"),cea:=xml.EA(class),start:=found.pos(1),end:=cea.end-cea.pos,rep:=replace:=SubStr(text,start,end)
					while(aa:=all.item[A_Index-1]),ea:=xml.EA(aa){
						underline:=""
						Loop,% StrLen(ea.text)
							underline.="_"
						rep:=RegExReplace(replace,"i)\b" ea.text "\b",underline)
					}StringReplace,text,text,%replace%,%rep%
					class.ParentNode.RemoveChild(class)
				}else{
					if(a="Function"&&found.1~="i)if|else|while")
						Continue
					if(a="Function"){
						m("REDO THIS! use 2225()")
						/*
							class:=SSN(current,"descendant::*[@type='Class' and @pos<=" position+found.pos(1) " and @end>=" position+found.pos(1) "]")
							if(method:=SSN(class,"descendant::*[@type='Method' and @upper='" Upper(found.1) "']")){
								if(tv:=SSN(method,"@cetv").text)
									Default("SysTreeView322"),TV_Delete(tv),Enable("SysTreeView322","CEGO")
								method.ParentNode.RemoveChild(method)
								Continue
							}
						*/
					}if(node:=SSN(current,"descendant::*[@text='" found.1 "' and @type='" a "']")){
						if(tv:=SSN(node,"@cetv").text)
							Default("SysTreeView322"),TV_Delete(tv)
						node.ParentNode.RemoveChild(node)
	}}}}}SetStatus(A_ThisFunc "() Processed In: " A_TickCount-tick "ms",3),Enable("SysTreeView322","CEGO")
}
RemoveItem(text,type,find){
	pos:=1,search:=v.OmniFindText[type]
	while(RegExMatch(text,find,found,pos)),pos:=found.pos(1)+found.len(1){
		/*
			t(A_ThisFunc,A_TickCount,"time:1")
		*/
		if(!found.len(1))
			Break
		if(type~="Bookmark|Breakpoint|Instance"){
			;ScanBBI(Current(5),text,type)
		}else if(type="Function"){
			if(found.1~="i)if|else|while")
				Continue
			m("REMOVE FUNCTION!!! but check to see if it a method first!","time:4")
		}else{
			node:=cexml.Find(main:=cexml.Find("//main/@file",Current(2).file),"descendant::file/@file",Current(3).file)
			m(node.xml)
			if(rem:=SSN(node,"descendant::*[@type='" type "' and @text='" found.1 "']")){
				if(tv:=SSN(rem,"@cetv").text)
					Enable("SysTreeView322"),Default("SysTreeView322"),TV_Delete(tv),Enable("SysTreeView322","CEGO")
				rem.ParentNode.RemoveChild(rem)
}}}}
...
Resize(info*){
	...
	if(i:=GuiKeep.current(A_Gui)){
		...
		for a,b in i.gui
			for c,d in b{
				if(c~="y|h")
					GuiControl,MoveDraw,%a%,% c hei+d-(c="y"?i.border:0)-120
				else
					GuiControl,MoveDraw,%a%,% c wid+d-200
			}t("this needs fixed","time:1")
		return
	}
	if(info.1="get"){
...
Save_Untitled(node,ask:=1){
	...
	if(text!=template){
		if(ask){
			if(m("The file " ea.file " Containing:",SubStr(text,1,100) (StrLen(text)>100?"...":""),"Has not been saved.  Save this file?","btn:ync","ico:!")!="Yes")
				return
		}
...
Save(option=""){
	...
	LineStatus.Save(),LineStatus.tv()
	SaveGUI(),vversion.save(1),LastFiles()
}
SaveGUI(win:=1){
...
ScanBBI(node,text,type){
	return t("I think this needs fixed bad","ScanBBI()","time:1")
	pos:=1,sc:=csc(),words:=[],find:=v.OmniFindText[type],main:=SSN(node,"ancestor-or-self::main"),SearchText:=sc.GetUNI()
	if(type=""||text="")
		return
	while(RegExMatch(text,v.OmniFind[type],found,pos)),pos:=found.pos(1)+found.len(1){
		if(!found.len(1))
			Break
		words[found.1]:=1
	}for a,b in words{
		RegExReplace(SearchText,find.1 a find.2,"",Left),RegExMatch(text,a,found),Left:=Round(Left)
		CurrentType:=type
		check:=SN(node,"descendant::*[@type='" type "' and @text='" a "']")
		if(check.length=Left)
			Continue
		if(check.length>Left){
			Loop,% check.length-Left
			{
				rem:=SN(node,"descendant::*[@type='" type "' and @text='" a "']").item[0],ea:=xml.EA(rem),rem.ParentNode.RemoveChild(rem)
				if(ea.cetv)
					Default("SysTreeView322"),TV_Delete(ea.cetv)
			}
		}if(Left>check.length){
			if(!header:=SSN(main,"header[@type='" Type "']/@cetv").text)
				Default("SysTreeView322"),cexml.Under(main,"header",{type:type,cetv:(header:=TV_Add(type,SSN(main,"@cetv").text))})
			Loop,% left-check.length
				Default("SysTreeView322"),cexml.Under(node,"info",{text:a,type:type,upper:Upper(a),cetv:TV_Add(a,header,"Sort Vis")})
		}
	}all:=sn(node,"descendant::*[@type='Bookmark']"),list:=""
	/*
		not removing duplicates.
	*/
	while(aa:=all.item[A_Index-1]),ea:=xml.ea(aa){
		prev:=Round(SN(aa,"preceding-sibling::*[@text='" ea.text "']").length),search:={Bookmark:[Chr(59) "#[","]"],Breakpoint:[Chr(59),"*[","]"],Variable:["",":="]},find:=search[ea.type]
		if(!InStr(SearchText,find.1 ea.text find.2,1,1,prev+1)){
			if(ea.cetv)
				Default("SysTreeView322"),TV_Delete(ea.cetv)
			aa.ParentNode.RemoveChild(aa)
		}
...
Toolbar_Editor(control){
	static tb,nw,tbid,cross,Visible,Current,all,color,ctrl,mea
	ctrl:=control,color:=RGB(settings.Get("//Toolbar_Editor/@highlight",0xEE00AA))
	if(ctrl.file)
		return ItemEA:=menus.EA("//*[@tv='" TV_GetSelection() "']"),ButtonEA:=settings.EA("//toolbar/bar[@id='" tbid "']/button[@func='" ItemEA.clean "']"),tb.ChangeIcon(ButtonEA.id,ctrl.icon-1,ctrl.file)
	...
	winname:=ea.win,current:=ea.toolbar,tb:=ToolBar.keep[mea.toolbar],tbid:=id
	...
	Toolbar_EditorClose:
	nw.SavePos(),nw.exit(),nw.Destroy(),remtv:=menus.SN("//*[@tv]")
	...
	if(TV_GetChild(tvitem)){
		...
		if(!settings.SSN("//toolbar/bar[@id='" tbid "']/button[@func='" ea.clean "']")){
			start:=9999
			while(settings.SSN("//toolbar/bar[@id='" tbid "']/descendant::*[@id='" ++start "']")){
			}new:=settings.Under(settings.SSN("//toolbar/bar[@id='" tbid "']"),"button",{file:"shell32.dll",func:ea.clean,icon:2,id:start,state:4,text:RegExReplace(ea.clean,"_"," "),vis:1}),tb.Add(nea:=xml.ea(new)),tb.AddButton(start),Default("SysTreeView321","Toolbar_Editor"),TV_Modify(A_EventInfo,"Select")
	}}else if(node:=settings.SSN("//toolbar/bar[@id='" tbid "']/button[@func='" ea.clean "']")){
		tb.Delete(xml.EA(node)),node.ParentNode.RemoveChild(node)
	}
	return
	TEHighlight:
	color:=RGB(clr:=Dlg_Color(settings.Get("//Toolbar_Editor/@highlight",0xEE00AA),nw.hwnd)),settings.Add("Toolbar_Editor",{highlight:clr}),Default("SysListView321","Toolbar_Editor"),LV_GetText(id,LV_GetNext()),ea:=MainWin.gui.ea("//controls[@win=1]/descendant::control[@id='" id "']"),winname:=ea.win,current:=ea.hwnd,tb:=ToolBar.keep[ea.hwnd],tbid:=id
	...
	*/
	...
	if(info.Delete)
		...
		if(ea.virtual)
			return
		if(!item)
			return ;t("You are not editing a script....","time:1")
		if(update[item]=text)
			return
		return update[item]:=text
		/*
			return updated[item]:=1,update[item]:=text
			if(updated[item]="")
...
UpPos(){
	...
	else if(sc.2008!=sc.2009&&sc.2507(6,sc.2008))
		sc.2505(0,sc.2006)
	lastpos:=sc.2008
}
...
Backspace(sub:=1){
	...
	sc.2078
	Loop,% sc.2570{
		cpos:=sc.2585(A_Index-1)-sub,chr:=Chr(sc.2007(cpos))
		if(sc.2579()!=sc.2577()){
			sc.2344
			Continue
		}
		if(match:=v.BraceDelete[chr]){
			check:=sub?1:0
			if(chr="%"){
				Send,{%send%}
				return sc.2079
			...
			if(chr~="(\(|\{|\[|\<)"){
				line:=sc.2166(cpos),matchline:=sc.2166(sc.2353(cpos))
				if(line!=matchline){
					if(chr="{"&&v.Options.Full_Auto){
						start:=line
						while((start:=sc.2225(line))>=0)
							line:=start
						end:=sc.2224(line,-1),sc.2645(cpos,1)
						FixLines(line,end-line,Floor(sc.2127(line)/settings.Get("//tab",5))),sc.Enable(1)
						Continue
					}sc.2645(cpos,1)
					Continue
				}else if(sub){
					if((flap:=Chr(sc.2007(sc.2585(A_Index-1))))=match)
						sc.2645(cpos,2)
				}else
					GoToPos(A_Index-1,cpos+1)
			}else if(sub){
				if(Chr(sc.2007(sc.2585(A_Index-1)))=match)
					sc.2645(cpos,2)
				else
					GoToPos(A_Index-1,cpos)
			}else{
				if(Chr(sc.2007(sc.2585(A_Index-1)-1))=match)
					sc.2645(cpos-1,2)
				else
					GoToPos(A_Index-1,cpos+1)
			...
			if(Chr(sc.2007(cpos+check))=match)
				sc.2645(cpos+check,2)
			else
				sc.2584(A_Index-1,cpos-(sub?0:-1)),sc.2586(A_Index-1,cpos-(sub?0:-1))
		}else
			sc.2645(cpos,1)
	}sc.2079
}
...
Check_For_Update(startup:=""){
	static newwin
	/*
		DownloadURL:="https://raw.githubusercontent.com/maestrith/AHK-Studio/master/AHK-Studio.ahk"
		VersionTextURL:="https://raw.githubusercontent.com/maestrith/AHK-Studio/master/AHK-Studio.text"
	*/
...
UnderlineDuplicateWords(){
	...
	if(!word)
		return
	while(found:=sc.2197(StrLen(word),[word]))>=0{
		dup.Insert(found),sc.2686(++found,sc.2006)
	}if(dup.MaxIndex()>1){
		for a,b in dup
			sc.2686(b,sc.2006),sc.2504(b,StrLen(word)),found++,list.=b "`n"
	}
}
class debug{
	static socket
	__New(){
		if(this.socket){
			...
			Sleep,500
			this.Disconnect()
		}debug.xml:=new XML("debug"),sock:=-1,debug.xml.Add("Local"),debug.xml.Add("Global")
		if(!v.debug.sc)
			MainWin.DebugWindow()
		DllCall("LoadLibrary","str","ws2_32","ptr"),VarSetCapacity(wsadata,394+A_PtrSize),DllCall("ws2_32\WSAStartup","ushort",0,"ptr",&wsadata),DllCall("ws2_32\WSAStartup","ushort",NumGet(wsadata,2,"ushort"),"ptr",&wsadata)
		OnMessage(0x9987,debug.Sock),socket:=sock
		...
		DllCall("ws2_32\freeaddrinfo","ptr",next),DllCall("ws2_32\WSAAsyncSelect","ptr",socket,"ptr",A_ScriptHwnd,"uint",0x9987,"uint",0x29),ss:=DllCall("ws2_32\listen","ptr",socket,"int",32),debug.socket:=socket 
		debug.id:=Current(3).id
	}
	TopXML(){
	...
	Run(filename){
		...
		if(WinExist("ahk_pid" pid)){
			...
			if(info.line&&info.file)
				SetPos({file:info.file,line:info.line})
		}return
	}
	...
	*/
	...
	Send(message){
		if(!debug.socket){
			t("There are currently no files being deubgged","time:1")
			Exit
		}
	...
	Receive(){
		...
		VarSetCapacity(packet,++length,0),received:=0,text:=""
		While(received<length){
			r:=DllCall("ws2_32\recv","ptr",socket,"ptr",&packet+received,"int",length-received,"int",0)
			if(r<1){
				error:=DllCall("GetLastError")
				return t(r,socket,length,received,"An error occured",error,"Possible reasons for the error:","1.  Sending OutputDebug faster than 1ms per message","2.  Max_Depth or Max_Children value too large","time:2")
			}else
				received+=r
		}
		Critical,Off
...
Debug_Current_Script(){
	...
	sc:=v.debug
	sc.2004(),sc.2003(0,"Initializing Debugger, Please Wait...`n")
	/*
	...
	*/
	...
	/*
		file:="D:\AHK\Duplicate Programs\AHK-Studio\Projects\Scratch Pad.ahk"
		debug.Run(file)
	*/
}
Run_Program(){
...
DisplayDebugInfo(){
	static DebugMarks:=[]
	sc:=csc(),Current:=Current(3).file
	list:=debug.XML.SN("//*[@name]"),sc.2080(10,14),sc.2500(10),sc.2082(10,0x0000FF)
	if(list.length)
		DebugMarks[Current]:=1
	else{
		if(DebugMarks[Current])
			sc:=csc(),sc.2500(10),sc.2505(0,sc.2006),DebugMarks.Delete(Current)
	}
	while(ll:=list.item[A_Index-1]),ea:=XML.EA(ll){
		if(ea.name){
			sc.2686(0,sc.2006)
			while((pos:=sc.2197((len:=StrPut(ea.name,"UTF-8")-1),[ea.name]))>=0)
				sc.2504(pos,len),sc.2686(pos+1,sc.2006)
}}}
GetChildren(){
	;children="1" numchildren="1"></pr
	obj:=v.GetChildren
	while(ea:=obj.pop()){
		t(ea.children,ea.numchildren,ea.scope)
		debug.Send("feature_set -n max_depth -v " ea.children)
		debug.Send("feature_set -n max_children -v " ea.numchildren)
		debug.Send("property_get -n " ea.fullname " -m " ea.numchildren " -i " ea.scope) ;maybe add a transaction?
		debug.Send("feature_set -n max_depth -v 0")
		debug.Send("feature_set -n max_children -v 0")
		Sleep,50
	}
}
VarBrowser(){
	...
	if(!WinExist(newwin.id))
		newwin:=new GUIKeep(98),newwin.add("TreeView,w300 h400 gvalue vtreeview AltSubmit hwndtreeview,,wh","Button,gStopDebug,Stop Debugging,y","Button,x+M gRefreshDebugVars,Refresh Variable List,y"),newwin.show("Variable Browser"),hwnd:=newwin.XML.SSN("//*/@hwnd").text,debug.VarBrowser:=1
	return
	98Close:
	98Escape:
	...
	if(A_GuiEvent!="Normal")
		return
	top:=debug.xml.SSN("//*")
	if(node:=SSN(top,"descendant::*[@tv='" A_EventInfo "']")){
		if(SSN(node,"wait")){
			node.SetAttribute("expanded",1)
			node.SetAttribute("ScanChildren",1)
			ScanDebugVars()
		}
	}
	return
...
ScanDebugVars(){
	...
	if(!debug.xml[]){
		m("here!!!!!!!!")
		exit
	}if(scan:=debug.xml.SSN("//*[@scan]")){
		ea:=xml.EA(scan),scan.RemoveAttribute("scan"),debug.Send("context_get -c " ea.id " -i " ea.where)
	}else if(scan:=debug.xml.SSN("//*[@context_names]")){
		ea:=xml.EA(scan),scan.RemoveAttribute("context_names"),debug.Send("context_get -c " ea.id " -i spoop!")
	}else if(scan:=debug.xml.SSN("//*[@ScanChildren]")){
		ea:=xml.EA(scan),scan.RemoveAttribute("ScanChildren"),ScanChildren.push(ea)
		SetTimer,ScanChildren,-100
		return
		ScanChildren:
		while(ea:=ScanChildren.pop()){
			debug.Send("feature_set -n max_depth -v " ea.children)
			Sleep,40
			debug.Send("feature_set -n max_children -v " ea.numchildren)
			Sleep,40
			debug.Send("property_get -n " ea.fullname " -m " ea.numchildren " -i " ea.scope)
			Sleep,% ea.numchildren*40
			debug.Send("feature_set -n max_depth -v 0")
			Sleep,40
			debug.Send("feature_set -n max_children -v 0")
			Sleep,50
...
InsertDebugMessage(){
	...
	if(!debug.VarBrowser){
		if(hotkey:=menus.SSN("//*[@clean='List_Variables']/@hotkey").text)
			sc.2003(sc.2006," : Press " Convert_Hotkey(hotkey) " to List Variables`n")
		else
			msg:=" : Click Here to Show the Variable List`n",end:=sc.2006(),sc.2003(end,msg),sc.2051(151,0xff0000),sc.2032(end+3,151),sc.2033(StrPut(msg,"UTF-8")-1,151),sc.2409(151,1)
	}
	sc.2025(sc.2006)
}
Display(){
	static recieve:=new xml("recieve"),total,width,addhotkey,lastid,StoreXML:=[],CV:={1:"Global",0:"Local"},c:=[],ProcessProperties:=[]
	start:=1,store:="",sc:=v.debug
	;xx:=new XML() ;for Studio
	while,v.displaymsg.1{
		store:=v.displaymsg.pop(),recieve.xml.LoadXML(store)
		if(info:=recieve.SSN("//stream[@type='stderr']")){
			info:=debug.decode(info.text),total.=info "`n",in:=StripError(info,v.debugfilename)
			if(in.file&&in.response/line)
		...
		if(init:=recieve.SSN("//init")){
			v.afterbug:=[],ad:=["stdout -c 1","stderr -c 1"],ea:=settings.ea("//features")
			for a,b in ["feature_set -n max_depth -v 0","feature_set -n max_children -v 0"]
				ad.Insert(b)
			...
			for a,b in ad
				v.afterbug.Insert(b)
			SetTimer,afterbug,-300
		}else if(node:=recieve.SSN("//response[@command='property_get' and @transaction_id!='']")){
			xx:=debug.xml,ea:=xml.EA(node)
			main:=xx.SSN("//*[@where='" ea.transaction_id "']")
			all:=SN(node,"//descendant::property")
			while(aa:=all.item[A_Index-1]),ea:=XML.EA(aa){
				if(A_Index=1){
					master:=SSN(main,"descendant-or-self::property[@fullname='" ea.fullname "']")
				}else{
					if(wait:=SSN(master,"descendant::wait")){
						Default("SysTreeView321",98)
						if(tv:=SSN(wait,"@tv").text)
							TV_Delete(tv)
						wait.ParentNode.RemoveChild(wait)
					}
					;XML.__Get.Name
					if(!next:=SSN(master,"descendant-or-self::property[@fullname='" ea.fullname "']")){
						next:=master.AppendChild(aa.CloneNode(1))
						;next.SetAttribute("tv",TV_Add(ea.fullname " = " debug.Decode(aa.text),SSN(master,"@tv").text))
					}else{
						for a,b in ea
							next.SetAttribute(a,b)
						nea:=xml.EA(next),text:=debug.Decode(aa.text)
						Default("SysTreeView321",98),TV_Modify(nea.tv,"",nea.name (text?" = " text:""))
						if(ea.type="Object"&&!next.HasChildNodes())
							Default("SysTreeView321",98),xx.Under(next,"wait",{tv:TV_Add("Please Wait...",SSN(next,"@tv").text)}),next.SetAttribute("scope",SSN(next,"ancestor::*/@where").text)
					}
					/*
						!!!!!!!!!!!!!!keep a list of ids for the breakpoints for real time deleting!!!!!!!!
						work on realtime adding
						have the debug line highlighted by something other than the highlight.
						<response command="breakpoint_set" transaction_id="" state="enabled" id="2"></response>
						object changing is causing an issue
						so if it was a non-obj and becomes an obj
						it fucks up
						like going from undefined to object.
					*/
					/*
						if(ea.fullname="XML.__Get.Name")
							m(aa.xml)
					*/
				}
			}
			not:=SN(main,"descendant::*[not(@tv)]")
			while(nn:=not.item[A_Index-1]),ea:=XML.EA(nn){
				Default("SysTreeView321",98)
				nn.SetAttribute("tv",TV_Add(ea.name " = " debug.Decode(nn.text),SSN(nn.ParentNode,"@tv").text))
			}
			ScanDebugVars()
		}else if(recieve.SSN("//response[@command='run' and @status='break' and @reason='ok']")){
			debug.Send("stack_get")
		}else if(recieve.SSN("//response[@command='run' and @status='stopped' and @reason='ok']")){
			/*
				ExitApp ;remove this before continuing :)
			*/
		}else if(recieve.SSN("//response[@command='context_names']")){
			xx:=debug.xml,scopes:=recieve.SN("//context")
			while(ss:=scopes.item[A_Index-1]),ea:=XML.EA(ss){
				node:=xx.SSN("//" ea.name)
				for a,b in {context_names:1,id:ea.id}
					node.SetAttribute(a,b)
			}ScanDebugVars()
		}else if(node:=recieve.SSN("//response[@command='context_get']")){
			all:=SN(node,"descendant-or-self::property"),pea:=xml.EA(node),xx:=debug.xml,main:=xx.SSN("//*[@where='" pea.transaction_id "']")
			while(aa:=all.item[A_Index-1]),ea:=XML.EA(aa){
				if(!property:=SSN(main,"descendant::property[@fullname='" ea.fullname "']"))
					property:=main.AppendChild(aa.CloneNode(0))
				else
					for a,b in ea
						property.SetAttribute(a,b)
				if(!tv:=SSN(property,"@tv").text){
					Default("SysTreeView321",98),property.SetAttribute("tv",(parent:=TV_Add(ea.fullname " = " debug.Decode(aa.text),SSN(property.ParentNode,"@tv").text,"Vis")))
					if(ea.children&&!SSN(property,"@expanded"))
						xx.Under(property,"wait",{tv:TV_Add("Please Wait...",parent)}),property.SetAttribute("scope",SSN(property,"ancestor::*/@where").text)
				}else{
					prop:=xml.EA(property)
					if(ea.children&&!SSN(property,"wait")&&!SSN(property,"@expanded")){
						Default("SysTreeView321",98),xx.Under(property,"wait",{tv:TV_Add("Please Wait...",prop.tv)}),property.SetAttribute("scope",SSN(property,"ancestor::*/@where").text)
						GuiControl,98:+Redraw,SysTreeView321
					}Default("SysTreeView321",98),TV_Modify(prop.tv,"",prop.fullname " = " debug.Decode(aa.text))
				}
				if(ea.children&&SSN(property,"@expanded").text){
					property.SetAttribute("ScanChildren",1)
					property.SetAttribute("id",pea.context)
				}
			}
			ScanDebugVars()
		}else if(recieve.SSN("//response[@command='stack_get']")){
			xx:=debug.xml
			stack:=recieve.SN("//stack")
			while(ss:=stack.item[A_Index-1]),ea:=XML.EA(ss){
				if(A_Index=1)
					stack:=ea
				if(!where:=xx.SSN("//Local/descendant::where[@where='" ea.where "']"))
					where:=xx.Under(xx.SSN("//Local"),"where",{where:ea.where})
				where.SetAttribute("scan",1)
				where.SetAttribute("id",ea.level)
				if(!SSN(where,"@tv").text)
					Default("SysTreeView321",98),where.SetAttribute("tv",TV_Add(ea.where))
			}
			file:=RegExReplace(RegExReplace(URIDecode(stack.filename),"\Qfile:///\E"),"\/","\")
			SetPos({file:file,line:stack.lineno-1})
			ScanDebugVars()
		}else if(recieve.SSN("//response[@command='step_into' and @status='break' and @reason='ok']")){
			debug.Send("stack_get")
		}else if(recieve.SSN("//response[@command='step_over']")){
			debug.Send("stack_get")
		}else if(recieve.SSN("//response[@command='step_out']"))
			debug.Send("stack_get")
		/*
			Step Over
			Step Into
			Step Out
			debugging errors just send them to the debug window
		*/
		if(v.Options.Verbose_Debug_Window)
			recieve.Transform(),recieve.Transform(),Debug(recieve[])
		return
	}
	afterbug:
