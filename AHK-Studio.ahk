...
Display(){
	...
	while,v.displaymsg.1{
		...
		if(init:=recieve.SSN("//init")){
			...
			main:=xx.SSN("//*[@scope='" ea.transaction_id "']")
			all:=SN(node,"//property")
			while(aa:=all.item[A_Index-1]),ea:=XML.EA(aa){
				if(A_Index=1){
					...
					if(!next:=SSN(master,"descendant::property[@fullname='" ea.fullname "']"))
						Default("SysTreeView321",98),next:=master.AppendChild(aa.CloneNode(1)),next.SetAttribute("tv",TV_Add(ea.fullname " = " debug.Decode(aa.text),SSN(master,"@tv").text))
					else{
						for a,b in ea
							next.SetAttribute(a,b)
						nea:=xml.EA(next)
			...
			v.debug.2003(sc.2006,"Execution Complete")
			/*
				m("DONE! :)")
			...
			*/
			...
			all:=SN(node,"property"),pea:=xml.EA(node),xx:=debug.xml,main:=xx.SSN("//*[@where='" pea.transaction_id "']")
			while(aa:=all.item[A_Index-1]),ea:=XML.EA(aa){
