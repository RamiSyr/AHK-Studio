...
Omni_Search(start=""){
	...
	newwin:=new GUIKeep(20),newwin.Add("Edit,goss w200 vsearch ReadOnly,,w","ListView,w200 h100 -hdr -Multi gosgo,Menu Command|Additional|1|2|Rating|index,wh")
	Gui,20:-Caption
	...
	if(InStr(search,"?")||search=""){
		...
		return
	}if(RegExMatch(search,"O)(\W)",found)){
		if(found.1="^")
			OnlyTop:=1
	...
	*/
	...
	while(ll:=list.Item[A_Index-1],b:=xml.ea(ll)){
		...
		if(!b.filename)
			b.filenameSplitPath(b.file).filename
		if(fsearch){
			...
			b.node:=ll,LV_Add("",b[info.1],b[info.2],(b[info.2]="Method"?b.class " : ":"") (info.3="file"?Trim(StrSplit(b[info.3],"\").pop(),".ahk"):b[info.3]),(v.Options.HasKey(b.clean)?(v.options[b.clean]?"Enabled":"Disabled"):b[info.4]),rating,++Index),Select[Index]:=b
	}
...
class Tracked_Notes{
	...
	__New(){
		...
		if(!this.XML.SSN("//master"))
			this.XML.Under(this.XML.Add("master",{file:"Global Notes"}),"global")
		this.Populate()
		return this
	}Register(sc){
		this.sc:=sc
	...
	Populate(){
		...
		project:=Current(2).file,file:=Current(3).file
...
/*
	...
	also if you are under an if/else/indent statement and you hit { it should <- the indent
	remove % auto move thingie
*/
