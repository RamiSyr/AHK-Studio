...
class debug{
	...
	__New(){
		...
		;----this rather than----
		;debug.id:=Current(3).id ;IMPORTANT
		;----the below 2 lines---
		ea:=xml.EA(cexml.Find("//file/@file","D:\AHK\Duplicate Programs\AHK-Studio\Projects\Scratch Pad.ahk"))
		debug.id:=ea.id
	}
	TopXML(){
	...
	while,v.displaymsg.1{
		...
		if(init:=recieve.SSN("//init")){
			...
			while(nn:=not.item[A_Index-1]),ea:=XML.EA(nn){
				Default("SysTreeView321",98)
				nn.SetAttribute("tv",TV_Add(ea.fullname " = " debug.Decode(aa.text),SSN(nn.ParentNode,"@tv").text))
			}
			ScanDebugVars()
			...
			*/
			...
			stack:=recieve.EA("descendant-or-self::stack")
