...
Color(con:=""){
	...
	*/
	con.2082(6,0xC08080),con.2198(0x2)
	con.2080(3,14)
	con.2680(3,6)
...
FileCheck(file:=""){
	...
	scidate:=20160512235500,XMLFiles:={menus:[20160708080312,"lib/menus.xml","lib\Menus.xml"],commands:[20160508000000,"lib/commands.xml","lib\Commands.xml"]}
...
VarBrowser(){
	...
	if(!WinExist(newwin.id))
		newwin:=new GUIKeep(98),newwin.add("TreeView,w300 h400 gvalue vtreeview AltSubmit hwndtreeview,,wh","Button,gStopDebug,Stop Debugging,y"),newwin.show("Variable Browser"),hwnd:=newwin.XML.SSN("//*/@hwnd").text,debug.VarBrowser:=1
	return
	98Close:
	98Escape:
...
Step_Into(){
	if(!debug.socket)
		return m("There is currently no script being debugged")
		...
		if(init:=recieve.SSN("//init")){
			...
			while(aa:=all.item[A_Index-1]),ea:=XML.EA(aa){
				if(A_Index=1){
					...
					}else if(SSN(next,"@type").text=ea.type){
						for a,b in ea
							next.SetAttribute(a,b)
						nea:=xml.EA(next)
						Default("SysTreeView321",98),TV_Modify(nea.tv,"",nea.name " = " debug.Decode(aa.text))
					}else{
						if(tv:=SSN(next,"@tv").text)
